...
export default function ResidentPortrait({ 
  ...
  // Adjusted size classes for full-body portraits
  const fullBodySizeClasses = {
    sm: 'w-16 h-32',
    md: 'w-24 h-48',
    lg: 'w-32 h-64'
  };
  ...
  // Get the appropriate size class based on portrait type
  const getContainerSizeClass = () => {
    if (showFullBody) {
      return fullBodySizeClasses[size];
    }
  ...
  return (
    <div className={`relative flex items-center justify-center ${className}`}>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          key={portraitType}
          className={`relative ${getContainerSizeClass()} overflow-hidden ${
