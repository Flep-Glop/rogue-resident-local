...
export default function TwitterStyleActivityWithTransition({ currentActivity, onComplete }: TwitterStyleActivityWithTransitionProps) {
  ...
  useEffect(() => {
    ...
    const sequence = async () => {
      ...
      // 4. Show brief context and start challenge
  ...
  useEffect(() => {
    ...
    const loadQuestions = async () => {
      if (loadingRef.current || hasLoadedQuestions || questions.length > 0) return;
      ...
      try {
        ...
        const activityQuestions = await selectActivityQuestions(
          currentActivity.domains[0] || KnowledgeDomain.RADIATION_THERAPY,
          currentActivity.difficulty,
          2,
          ['multipleChoice'],
          currentActivity.mentor || 'Kapoor'
        );
        
        if (activityQuestions.length > 0) {
          setQuestions(activityQuestions);
          // Add initial mentor message with first question
          setTimeout(() => {
            addMentorMessage(activityQuestions[0]);
          }, showPatientTransition ? 500 : 800);
        }
      } catch (error) {
        console.error('[TwitterStyleActivityWithTransition] Error loading questions:', error);
      } finally {
        setIsLoading(false);
  ...
  // Rest of the TwitterStyleActivity logic (message handling, etc.)
  const addMentorMessage = (question: Question) => {
    ...
    setMessages(prev => {
      ...
      return [...prev, {
        id: `mentor-${question.id}-${Date.now()}-${Math.random()}`,
        type: 'mentor',
  ...
  const getStarPointChance = (): number => {
    ...
    const momentumBonus = momentum * 0.05;
    return Math.min(baseChance + momentumBonus, 0.25);
  };
  ...
  if (isLoading && animationPhase === 'challenge-ready') {
    return (
      <div style={{
        width: '100vw',
        height: '100vh',
    ...
    <div style={{
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          {/* Typing indicator */}
          {showTypingIndicator && animationPhase === 'challenge-ready' && (
            <div style={{
              backgroundColor: '#3a3a4e',
              border: `2px solid ${colors.border}`,
              borderRadius: spacing.sm,
